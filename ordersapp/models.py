...
class Order(models.Model):
    ...
    def delete(self):
        for item in self.orderitems.select_related():
            item.product.quanity += item.quantity
            item.product.save()
...
class OrderItemQuerySet(models.QuerySet):
    def delete(self, *args, **kwargs):
        for object in self:
            object.product.quanity += object.quantity
            object.product.save()
...
class OrderItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            self.product.quanity -= self.quantity - self.__class__.get_item(self.pk).quantity
        else:
            self.product.quanity -= self.quantity
        self.product.save()
